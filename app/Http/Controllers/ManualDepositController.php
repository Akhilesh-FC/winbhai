...
{
    public function manual_deposit_index(string $id)
    {
         $deposits= DB::select("SELECT payins.*,users.username AS uname,users.id As userid, users.mobile As mobile FROM `payins` LEFT JOIN 
            users ON payins.user_id=users.id WHERE payins.status = '$id' && payins.type=2");

        return view('manual_deposit.deposit')->with('deposits',$deposits)->with('id',$id);
    
}
public function manual_success(string $id) {
    // Fetch the details
 ...
 public function manual_reject(string $id){
