	   ...
	   public function aviator_bet(Request $request)
{
    ...
    try {
        ...
        /** ✅ CASE 1: Third party wallet sufficient */
        if ($user->third_party_wallet >= $thirdPartyDeduct) {

            // ✅ winning wallet priority (only wallet part)
            if ($user->winning_wallet >= $walletDeduct) {
                DB::table('users')->where('id', $uid)->update([
                    'wallet'             => DB::raw("wallet - $walletDeduct"),
                    'winning_wallet'     => DB::raw("winning_wallet - $walletDeduct"),
        ...
        /** ✅ CASE 2: Third party insufficient → full wallet */
        else {
        ...
        // ✅ commission logic
        $betting_fee   = DB::table('admin_settings')->where('id', 10)->first();
        $percentage    = $betting_fee->longtext ?? 0;
        $commission    = $amount * $percentage;
        $tradeAmount   = $amount - $commission;

        // ✅ insert bet
        DB::table('aviator_bet')->insert([
            'uid'             => $uid,
            'amount'          => $amount,
            'number'          => $number,
            'game_id'         => $game_id,
